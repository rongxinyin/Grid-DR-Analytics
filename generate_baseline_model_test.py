...
for item in model_params:
    ...
    commandLine = (
        OpenStudioPath + ' run -m -w '
        + '"{}/osw_output/{}.osw"'.format(file_path, item['BaseModelId'])
    )
    ...
    idf_file = (
        file_path
        + '/osw_output/eplus/{}.idf'.format(item['BaseModelId'])
    )
    ...
    osm_file = (
        file_path
        + '/osw_output/eplus/{}.osm'.format(item['BaseModelId'])
    )
